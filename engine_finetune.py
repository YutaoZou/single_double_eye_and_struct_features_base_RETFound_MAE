...
from sklearn.metrics import (
    classification_report, accuracy_score, roc_auc_score, f1_score, average_precision_score,
...
def train_one_epoch(
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, f'Epoch: [{epoch}]')):
        # 在engine_finetune.py的train_one_epoch函数中添加
#     for data_iter_step, (samples, targets) in enumerate(...):
        # 每5个批次打印一次类别分布（假设抑郁标签是0，非抑郁是1）
        if data_iter_step % 5 == 0:
            depression_cnt = (targets == 0).sum().item()  # 抑郁样本数
            non_depression_cnt = (targets == 1).sum().item()  # 非抑郁样本数
            total = depression_cnt + non_depression_cnt
            print(f"批次类别分布: 抑郁{depression_cnt}个 ({depression_cnt/total*100:.1f}%), 非抑郁{non_depression_cnt}个    ({non_depression_cnt/total*100:.1f}%)") 
